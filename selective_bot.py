...
class SelectiveBot:
    ...
    def play_round(self, winner, win_amount):
        ...
        if winner != -1:
            self.winning_behaviour_inturn= win_amount / (self.players_money[winner] - 500) #the behaviour of the recent winner
            if self.winning_behaviour_inturn != 1 and self.winning_behaviour_inturn != 0.5 and self.winning_behaviour_inturn!=0:
                ...
                self.sumof_bit_bellow_values += (self.players_money[winner] - 500) - win_amount #the needed bit bellow value to win
        ...
        max_wins=max([self.behaviours[key][0] for key in self.keys]) #the max_wins that have been with 1 or more behaviours
        winnable_behaviours=[] #saves all the behaviours that won max_wins times.

        for key in self.keys:
            if self.behaviours[key][0]==max_wins: winnable_behaviours.append(key)
